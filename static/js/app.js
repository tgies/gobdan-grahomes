...
async function submitMessage() {
  ...
  const cipherPair = document.getElementById('cipherDropdown').value;
  ...
  try {
    const response = await fetch('/api/encode', {
      ...
      body: JSON.stringify({ message, temperature, top_p: topP, model: selectedModel, cipher_pair: cipherPair })
    });
    if (!response.ok) {
...
async function decodeMessage() {
  ...
  try {
    const response = await fetch('/api/decode', {
      ...
      body: JSON.stringify({ encodedMessage, temperature, top_p: topP, model: selectedModel, cipher_pair: cipherPair })
    });
    if (!response.ok) {
      throw new Error('Error from model response');
    }
...
function changeCipherPair() {
  const cipherDropdown = document.getElementById('cipherDropdown');
  const selectedCipher = cipherDropdown.value;

  // Clear all relevant fields except model and sampling settings
  document.getElementById('localDecodedMessage').value = '';
  document.getElementById('hiddenEncodedMessage').value = '';
  document.getElementById('foolsRavings').value = '';
  document.getElementById('gptDecodedMessage').value = '';

  // Update placeholder text based on selected cipher pair
  if (selectedCipher === 'atbash') {
    document.getElementById('localDecodedMessage').placeholder = "Cipher of the Grackle";
    document.getElementById('gptDecodedMessage').placeholder = "Cipher of the Bard";
  } else if (selectedCipher === 'rot13') {
    document.getElementById('localDecodedMessage').placeholder = "Cipher of the Drunkard";
    document.getElementById('gptDecodedMessage').placeholder = "Cipher of the Soothsayer";
  }
}
